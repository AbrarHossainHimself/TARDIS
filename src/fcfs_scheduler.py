...
class FCFSScheduler:
    def __init__(
        ...
        self.queued_jobs = []  # Initialize queued_jobs list
    ...
    def simulate(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
        ...
        # Sort jobs by submission time (FCFS)
    ...
    def _handle_submission(self, job: Dict) -> None:
        """Handle job submission with pure FCFS scheduling"""
        if self.can_schedule_job(job):
            self._start_job(job)
        else:
            # If can't schedule now, add to queue maintaining FCFS order
            self.queued_jobs.append(job)

    def _handle_completion(self, job: Dict) -> None:
        """Handle job completion and try to schedule queued jobs in FCFS order"""
        if job['job_id'] in self.running_jobs:
            ...
            # Try to schedule queued jobs in FCFS order
            if self.queued_jobs:
                remaining_jobs = []
                # Try each job in order, maintaining FCFS
                for queued_job in self.queued_jobs:
                    if self.can_schedule_job(queued_job):
                        self._start_job(queued_job)
                    else:
                        remaining_jobs.append(queued_job)
    ...
    def _start_job(self, job: Dict) -> None:
        ...
        # Calculate wait time from original submit time
        original_submit = job.get('original_submit_time', job['submit_time'])
        job['wait_time'] = (job['start_time'] - original_submit).total_seconds()
